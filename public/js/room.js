...
document.addEventListener("DOMContentLoaded", function () {
    const chatCont = document.querySelector(".container-room .right-cont .chat-cont");
    const colorPicker = document.getElementById("colorPicker");

    colorPicker.addEventListener("input", function () {
        const selectedColor = colorPicker.value;
        chatCont.style.backgroundColor = selectedColor;
    });
});
...
let currentRoomUsernames = {};
socket.on('join room', async (conc, cnames, micinfo, videoinfo) => {
    currentRoomUsernames = cnames || {};
    userList.innerHTML = ''; // Clear the previous user list

if (currentRoomUsernames) {
    Object.keys(currentRoomUsernames).forEach((sid) => {
        const listItem = document.createElement('li');
        listItem.textContent = `${currentRoomUsernames[sid]}`;
        userList.appendChild(listItem);
    });
}
...
function updateAttendanceList() {
  attendanceList.innerHTML = '<h2>Attendance List</h2>';
  for (const username in attendance) {
    const listItem = document.createElement('li');
    listItem.textContent = `${username}: ${attendance[username]}`;
    attendanceList.appendChild(listItem);
  }
}

// Event listener for the attendance button
attendanceButton.addEventListener('click', () => {
  if (username) {
    attendance[username] = 'Present';
    updateAttendanceList();
  } else {
    alert('Please enter your name to mark attendance.');
  }
});
